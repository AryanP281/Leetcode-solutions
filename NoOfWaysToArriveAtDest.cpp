...
/*
Let G be the goal node, S be the start node, dist[i] be the min length of path from S to node i. Consider for a neighbour node N of G, 
a path of min length (S->G) passes through N. This path is a superset of only the min paths to N (S->N) because - 
1.  There is only 1 edge (E) from N to G
2.  We know that dist[N] + E >= dist[G], since if dist[N] + E would have been less than dist[G], 
    then dist[G] would have been dist[N]+E itself as it is supposed to be the min path length to G
3.  Any path to N other than the min path will have length (L) > dist[N]. 
    Thus, even if dist[N] + E = dist[G], L + E will be > dist[G] since E > 0. 
    Hence, any path to N, other than the min, cannot be a part of a min path to G (S->G)
Thus, only the min path to N (S->N) can be a subset of the min path to G (S->G).
This logic can be extended for all other nodes such that the min path to any node is the superset of only the min paths to its neighours.
Therefore, for any node I, ways[I] = ways[M], where M are neighbours of I such that dist[M] + E(M->I) = dist[I].
*/
...
typedef pair<int64_t,int64_t> Vec;
...
int64_t countPaths(int64_t n, vector<vector<int64_t>>& roads) 
{
    vector<vector<Vec>> adjList(n, vector<Vec>());
    for(int i = 0; i < roads.size(); ++i)
    {
        adjList[roads[i][0]].push_back(Vec(roads[i][1], roads[i][2]));
        adjList[roads[i][1]].push_back(Vec(roads[i][0], roads[i][2]));
    }

    int64_t* minTimes = dijkstras(adjList);
    bool* visited = new bool[adjList.size()]();
    int64_t* dp = new int64_t[adjList.size()];
    dp[0] = 1;
    fill_n(dp+1, adjList.size()-1,-1);

    int64_t paths = getPaths(adjList.size()-1, adjList, minTimes, visited, dp);
...
int64_t* dijkstras(vector<vector<Vec>>& adjList)
{
    int64_t* minDists = new int64_t[adjList.size()]();
    fill_n(minDists, adjList.size(), LLONG_MAX);
    minDists[0] = 0;
    ...
    while(!heap.empty())
    {
        ...
        if(currNode.second == minDists[currNode.first])
        {
            int64_t v = currNode.second;
            pr++;
            for(int i = 0; i < adjList[currNode.first].size(); ++i)
            {
                if(currNode.second + adjList[currNode.first][i].second < minDists[adjList[currNode.first][i].first])
                {
                    minDists[adjList[currNode.first][i].first] = currNode.second + adjList[currNode.first][i].second;
                    heap.push(Vec(adjList[currNode.first][i].first, minDists[adjList[currNode.first][i].first]));
                }
            }
        }
    }
...
int64_t getPaths(int64_t i, vector<vector<Vec>>& adjList, int64_t* minDists, bool* visited, int64_t* dp)
{
    ...
    {
        if(minDists[adjList[i][j].first] + adjList[i][j].second == minDists[i] && !visited[adjList[i][j].first])
            dp[i] = (dp[i] + getPaths(adjList[i][j].first, adjList, minDists, visited, dp)) % MOD;
    }
